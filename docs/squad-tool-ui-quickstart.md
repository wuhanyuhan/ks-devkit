...
**适用读者**：要给 squad 加 widget UI 的开发者。本文按"用什么 SDK → 几行代码 → 跑通"组织，覆盖 ksapp Go SDK（`github.com/wuhanyuhan/ks-devkit/sdk/go/ksapp`）、ks-app Python SDK（PyPI `ks-app-sdk`，导入名 `ks_app`）、ks-squad-framework（`github.com/wuhanyuhan/ks-squad-framework/bootstrap`）。协议层契约（URI 形态、字段含义、版本规则、错误码）见 [`ks-types/docs/widgets-protocol-v1.md`](https://github.com/wuhanyuhan/ks-types/blob/master/docs/widgets-protocol-v1.md)；本文只讲 squad 端 SDK 调用。
...
iframe 内 SDK 见 `@wuhanyuhan/squad-widget-sdk` npm 包；也可用裸 `postMessage`（方法名常量见上）。
...
- 测试模板：`sdk/go/ksapp/app_register_tool_test.go`、`sdk/python/tests/test_meta_with_ui.py`、`ks-squad-framework/bootstrap/app_meta_with_ui_test.go`
